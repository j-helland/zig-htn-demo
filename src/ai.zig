...
fn __lessThan(context: *const math.Vec2(f32), a: Pair, b: Pair) std.math.Order {
    return std.math.order(context.sqDistRect(a.rect), context.sqDistRect(b.rect));
}
const DistancePriorityQueue = std.PriorityQueue(Pair, *const math.Vec2(f32), __lessThan);
...
pub fn oFindNextCoverPointToSearch(
    ...
    var queue = DistancePriorityQueue.init(gameState.allocator, &entityPoint);
    defer queue.deinit();
...
pub fn oFindCover(
    ...
    const playerPosition = gameState.ecs.componentManager.get(player, game.Position)
        orelse position;
...
pub fn oNavTo(
    ...
    if (ai.playerVisibilityMap == null) {
        ...
        // the AI to not "cheat".
        nav.pathfindVisibilityBias(
            initCellId, targetCellId, &gameState.navMeshGrid, &gameState.blockedCells, &ai.playerVisibilityMap.?, &path);
    }
